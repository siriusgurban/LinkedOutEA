...
function renderElements(data) {
    ...
    <div class="card-body">
      ...
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-warning btn-sm" onclick="handleEditEl(${post.id})">Edit</button>
        <input type="text" class="form-control dataId" data-id="${post.id}" data-title="${post.title}" value="${post.body}"/>
      </div>
    </div>
  </div>
    `;
...
async function handleEditEl(id) {
    try {
        ...
        dataId.forEach(el => {
            if (id == parseInt(el.dataset.id)) {
                uptPost(id, { title: el.dataset.title, body: el.value })
            }
        })

        

        console.log("Ugurla editlendi?)", id);
